...
<head>
    ...
    <style>
        ...
        .add-form { background: white; border-radius: 15px; padding: 30px; margin-bottom: 30px; box-shadow: 0 5px 20px rgba(0,0,0,0.08); }
    </style>
</head>
<body>
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="<?= base_url('admin/compatibility') ?>" class="sidebar-item active"><i class="fas fa-link me-3"></i>Compatibility</a>
    ...
    <div class="main-content">
        <div class="page-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-1">Manage Compatibility</h1>
                    <p class="text-muted mb-0">Define which components are compatible with each level</p>
                </div>
        ...
        <div class="add-form">
            <h5 class="mb-3"><i class="fas fa-plus-circle me-2"></i>Add Compatibility</h5>
            <form action="<?= base_url('admin/add-compatibility') ?>" method="POST">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="level_id" class="form-select" required>
                            ...
                            <?php foreach($levels as $level): ?>
                                <option value="<?= $level['id'] ?>"><?= esc($level['name']) ?> (฿<?= number_format($level['base_price'], 2) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="component_id" class="form-select" required>
                            ...
                            <?php foreach($components as $comp): ?>
                                <option value="<?= $comp['id'] ?>"><?= esc($comp['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <button type="submit" class="btn-primary-custom w-100"><i class="fas fa-plus me-2"></i>Add</button>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="data-table">
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php if($compatibilities && count($compatibilities) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    foreach ($levels as $lvl) {
                                        if ($lvl['id'] == $compat['level_id']) { $lvlName = $lvl['name']; break; }
                                    }
                                    ?>
                                    <span class="badge bg-primary"><?= esc($lvlName) ?></span>
                                </td>
                                <td>
                                    <?php
                                    $compName = 'Unknown';
                                    foreach ($components as $comp) {
                                        if ($comp['id'] == $compat['component_id']) { $compName = $comp['name']; break; }
                                    }
                                    ?>
                                    <?= esc($compName) ?>
                                </td>
                                <td>
                                    <a href="<?= base_url('admin/remove-compatibility/' . $compat['id']) ?>" class="btn-action btn-delete" onclick="return confirm('Remove this compatibility?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="text-center py-4">
                                    ...
                                    <p class="text-muted">No compatibility mappings found.</p>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
