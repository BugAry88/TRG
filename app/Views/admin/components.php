...
<head>
    ...
    <style>
        ...
        .btn-action { padding: 6px 12px; border-radius: 6px; text-decoration: none; margin: 0 2px; transition: all 0.3s ease; }
        ...
        .btn-primary-custom { background-color: var(--linn-teal); color: white; border: none; padding: 10px 20px; border-radius: 8px; text-decoration: none; transition: all 0.3s ease; }
        ...
        .component-thumb {
            ...
            border: 1px solid #e9ecef; padding: 4px;
        }
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-1">Manage Components</h1>
                    <p class="text-muted mb-0">Configure LP12 turntable components</p>
                </div>
                <div>
                    <a href="<?= base_url('admin/create-component') ?>" class="btn-primary-custom me-2"><i class="fas fa-plus me-2"></i>Add Component</a>
                            ...
                            <tr>
                                <td>
                                    <?php if(!empty($component['image_path'])): ?>
                                        <?php $imgSub = preg_replace('#^/?images/#', '', $component['image_path']); ?>
                                        <img data-imgpath="<?= esc($imgSub) ?>" alt="<?= esc($component['name']) ?>" class="component-thumb lazy-img" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                                        <div class="component-thumb-placeholder" style="display:none"><i class="fas fa-image"></i></div>
                                    <?php else: ?>
                                        <div class="component-thumb-placeholder"><i class="fas fa-image"></i></div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <strong><?= esc($component['name']) ?></strong>
                                    <?php if(!empty($component['description'])): ?>
                                        <br><small class="text-muted"><?= esc($component['description']) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php
                                    ...
                                    if (isset($categories)) {
                                        foreach ($categories as $cat) {
                                            if ($cat['id'] == $component['category_id']) {
                                                $catName = $cat['name'];
                                                break;
                                            }
                                        }
                                    ...
                                    ?>
                                    <span class="badge bg-info"><?= esc($catName) ?></span>
                                </td>
                                <td>฿<?= number_format($component['price_modifier'], 2) ?></td>
                                <td>
                                    <a href="<?= base_url('admin/edit-component/' . $component['id']) ?>" class="btn-action btn-edit"><i class="fas fa-edit"></i></a>
                                    <a href="<?= base_url('admin/delete-component/' . $component['id']) ?>" class="btn-action btn-delete" onclick="return confirm('Are you sure?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center py-4">
                                    ...
                                    <p class="text-muted">No components found. <a href="<?= base_url('admin/create-component') ?>">Add your first component</a></p>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    <script>
        // Build base URL from actual browser location (avoids case mismatch with PHP baseURL config)
        const pathParts = window.location.pathname.split('/');
        // Remove 'admin' and 'components' segments to get base path
        ...
        document.querySelectorAll('img.lazy-img').forEach(function(img) {
            const imgPath = img.getAttribute('data-imgpath');
            if (imgPath) {
                img.src = baseUrl + 'img/' + imgPath;
            }
        });
    </script>
</body>
